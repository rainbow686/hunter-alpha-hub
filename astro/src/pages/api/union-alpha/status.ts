...
import { UNION_ALPHA_MODEL_ID } from "@repo/lib/union-alpha";

/**
 * GET /api/union-alpha/status
 *
 * Live status for the Union Alpha stealth model, read from the OpenRouter public
 * catalogue. Same path and same JSON shape as the Next route it replaces, so the
 * page's polling client does not care which app serves it.
 */
...
export const GET: APIRoute = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      return json({ online: null, checkedAt, error: `OpenRouter returned ${response.status}` }, 502);
    }
    ...
    return json(
      { online: null, checkedAt, error: error instanceof Error ? error.message : "Unknown fetch failure" },
      502,
    );
  }
};
