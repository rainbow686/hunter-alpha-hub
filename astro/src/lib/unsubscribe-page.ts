/**
 * The unsubscribe page, as one self-contained HTML document.
 *
 * Why hand-written HTML instead of a rendered layout: the two deployments that
 * can answer this URL (`astro` and the retired-in-waiting Next app) do not share
 * a component system, and this page has a harder constraint than any other page
 * on the site — it is opened from an email, in a mail client's in-app browser, on
 * a phone, often on a bad connection, and sometimes fetched by a link scanner
 * with no JavaScript at all. So: no scripts, no external stylesheet, no webfont,
 * no image. One document that renders the same everywhere.
 *
 * The palette below is the frozen system (`src/styles/system.css`, ADR-0011)
 * inlined. Inlining means it can drift, so `scripts/check-subscription.mjs`
 * compares every token declared here against system.css and fails on a
 * difference — the same reason the two night blocks are compared there.
 */
